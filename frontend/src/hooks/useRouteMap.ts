...
export const useRouteMap = (
    ...
    const buildRoute = useCallback(() => {
        ...
        if (!routeData?.checkpoints || routeData.checkpoints.length < 2) {
            console.log('Cannot build route: not enough checkpoints', routeData?.checkpoints?.length);
            return null;
        }
        ...
        try {
            ...
            const points = routeData.checkpoints.map((cp) => [cp.latitude, cp.longitude]);
    ...
    const addUserMarker = useCallback(() => {
        if (!ymaps || !map || !userLocation) {
            console.log('Cannot add user marker:', { ymaps: !!ymaps, map: !!map, userLocation: !!userLocation });
            return null;
        }
