...
export const resetPassword = createAsyncThunk<boolean, { password: string; token: string }, { rejectValue: string }>(
    'user/resetPassword',
    async (data, { rejectWithValue }) => {
...
export const forgotPassword = createAsyncThunk<boolean, { email: string }, { rejectValue: string }>(
    'user/forgotPassword',
    async (data, { rejectWithValue }) => {
...
export const fetchUser = createAsyncThunk<TUser, void, { rejectValue: string }>(
    ...
    async (_, { rejectWithValue }) => {
        ...
        if (!response?.success) {
            return rejectWithValue(response.message || 'Ошибка получения пользователя');
        }
...
export const updateUser = createAsyncThunk<TUser, TUpdateUserData, { rejectValue: string }>(
    'user/update',
    async (data, { rejectWithValue }) => {
